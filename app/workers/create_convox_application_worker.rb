class CreateConvoxApplicationWorker < BaseWorker
  ...
  def perform(app_id)
    ...
    @items = [
      "git clone git@github.com:david-harkness/terminus.git --depth=1 ./ || git fetch && git checkout #{@convox_app.branch}",
      "convox login console.convox.com",
      "convox switch #{@the_rack}",
      "convox apps create #{@app_name}  --wait",
      "convox env set campaigns=2 tactics_per_campaign=2 accounts=3 daily_stats=3 account_stats=3 device_stats=3 geographic_stats=2 site_stats=3 time_stats=3 creative_stats=4",
      "convox env set RAILS_ENV=pr RACK_ENV=pr --app #{@app_name} --wait",
      "convox deploy --file docker-compose.pr.yml --app #{@app_name}",
